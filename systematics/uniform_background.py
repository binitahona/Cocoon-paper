...
spectrum0 = Powerlaw()
shape0    = Disk_on_sphere()

source0   = ExtendedSource("flat",
...
spectrum0.K.bounds = (1e-18 * fluxUnit, 1e-8*fluxUnit)
...
spectrum0.index.bounds = (-5., 0.)

# Set spatial parameters (do it after the source definition to make sure
# the units are handled correctly)
shape0.lon0 =  307.17 * u.degree
shape0.lon0.fix = True

shape0.lat0 = 41.17 * u.degree
shape0.lat0.fix = True

shape0.radius = 8 * u.degree
shape0.radius.fix = True
...
lm = Model(source0, source, source2, source3)
...
TS = jl.compute_TS("flat", like_df)
print("TS:\n")
print(TS)


jl.get_errors()
jl.results.write_to("flat_backgroun.fits", overwrite=True)
lm.save("flat_background.yml", overwrite=True)
