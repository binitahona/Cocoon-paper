...
# gamma-ray spectrum in the burst scenario
def phi_gamma_burst_p(E_gamma, k, Q_inj_index, E_inj_max):
    phi_gamma_ = E_gamma * 0.
    for i, E_gamma_ in enumerate(E_gamma):
        E_p_max = E_inj_max * 10. if E_gamma_ < E_inj_max else E_gamma_ * 10.
        logE_p, dlogE_p = np.linspace(np.log10(E_gamma_*(1+1e-10)), np.log10(E_p_max), 2049, retstep=True) # have integration lower limit slightly above E_gamma to avoid dividing 0 in lamdba calculation
        E_p = 10.**logE_p
        phi_gamma_[i] = integrate.romb(y=dN_dE_t0_p(E_p, t_burst, k, Q_inj_index, E_inj_max) * sigma_pp_inelastic(E_p) * F_gamma(E_gamma_ / E_p, E_p), dx=dlogE_p) * c_cgs * n_gas * np.log(10.) * dt_burst
